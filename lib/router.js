(function() {
  ...
  Router = function(options) {
    ...
    dispatch = function(req, res) {
      var args, full_path, home_page, index, m, method, param, parsed, pathname, route, _i, _j, _len, _len2, _len3, _ref, _ref2, _ref3;
      ...
      if (pathname === "/") {
        ...
        for (_j = 0, _len3 = _ref3.length; _j < _len3; _j++) {
          ...
          try {
            ...
            if (!!dispatch.logging) dispatch.log(error.toString());
          }
        }
    ...
    _bodyparser = function(body) {
      if (body.indexOf('=') !== -1) {
        try {
          ...
          if (!!dispatch.logging) dispatch.log(e);
        }
      }
      try {
    ...
    dispatch.static = function(pathname, res) {
      ...
      return fs.stat(full_path, function(err, stats) {
        var fd;
        if (err) {
          if (!!dispatch.logging) dispatch.log(err.toString());
    ...
    dispatch.cgi = function(pathname, res) {
      var child, full_path;
      try {
        full_path = "" + dispatch.static_route + pathname;
