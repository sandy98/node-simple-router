...
(function() {
  ...
  Router = function(options) {
    var default_options, dispatch, domain, escaped_icon, fs, mime_types, path_tools, querystring, spawn, urlparse, _bodyparser, _dirlist_template, _extend, _make_request_wrapper, _multipartparser, _parsePattern, _pushRoute;
    if (options == null) {
      options = {};
    }
    ...
    dispatch["static"] = function(pathname, req, res) {
      ...
      return fs.exists(full_path, function(exists) {
        if (exists) {
          if ((pathname.indexOf("" + dispatch.cgi_dir + "/") !== -1) && (pathname.substr(-1) !== "/") && (dispatch.serve_cgi === true)) {
            try {
    ...
    dispatch.cgi = function(pathname, req, res) {
      var body, child, d, data, env, full_path, respbuffer, urlobj;
      urlobj = urlparse(req.url);
      if (!!dispatch.logging) {
        dispatch.log(JSON.stringify(urlobj));
      }
      respbuffer = '';
      full_path = "" + dispatch.static_route + (unescape(pathname));
