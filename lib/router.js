...
(function() {
  ...
  Router = function(options) {
    var default_options, dispatch, domain, escaped_favicon, escaped_icons_png, fs, http, mime_types, net, nsr_sessions, padString, path_tools, querystring, spawn, thousandSep, urlparse, uuid, _bodyparser, _extend, _getSessionHandler, _isEmpty, _make_request_wrapper, _multipartparser, _parsePattern, _pushRoute;
    if (options == null) {
      options = {};
    }
    ...
    thousandSep = function(num, sep) {
      ...
      return num.toString().split('').reverse().join('').replace(/(\d{3})/g, "$1" + sep).split('').reverse().join('');
    };
