...
(function() {
  ...
  Router = function(options) {
    ...
    dispatch = function(req, res) {
      var args, error, full_path, home_page, index, m, method, param, parsed, pathname, route, selected_method, _i, _j, _k, _len, _len1, _len2, _ref, _ref1;
    ...
    */
    ...
    dispatch.compile_template = function(template_string, context, keep_tokens) {
      var dont_encode, full_text, html_encode, index, item, k, key, new_str, property, ret_arr, section, section_inner_text, section_pattern, section_pattern_global, section_tokens, section_type, sections, stripped_string, text, text_token, text_tokens, token, token_obj, value, variable_pattern, variable_pattern_global, variable_tokens, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m;
      if (keep_tokens == null) {
        keep_tokens = false;
      }
      ...
      for (index = _j = 0, _len1 = text_tokens.length; _j < _len1; index = ++_j) {
        ...
        for (key in context) {
          value = context[key];
          if (token_obj[key]) {
            new_str = new_str.replace(new RegExp(token_obj[key].token, 'g'), token_obj[key].dont_encode ? value : html_encode(value));
          }
        }
        if (!keep_tokens) {
      ...
      for (index = _l = 0, _len3 = text_tokens.length; _l < _len3; index = ++_l) {
        ...
        if (section_tokens[index]) {
          ...
          if (section_type === false) {
            if (!property) {
              ret_arr.push(dispatch.compile_template(section_inner_text, context));
            }
          } else {
